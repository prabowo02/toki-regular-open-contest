...
int main(){
	...
	rep(x,0,q){
		rep(z,0,4){
			...
			if (m.count(ii(i2,j2))){
				v.pub({m[ii(i,j)]-x,m[ii(i,j)],z});
				v.pub({m[ii(i2,j2)]-x,m[ii(i2,j2)],z^1});
			}
		}
		m.erase(dq.front());
		dq.pof();
	...
	rep(x,0,n){
		rep(z,0,4){
			...
			if (m.count(ii(i2,j2))) v.pub({m[dq[x]]-q,m[dq[x]],z});
		}
	}
	...
	rep(x,0,q) if (clap[x]=='1') rep(y,0,4) bs2[x*4+y+4]=1;
	
	rep(x,0,n){
		while (!v.empty() && v.back().time==x){
			bs.flip((v.back().pos-x)*4+v.back().dir);
			v.pob();
		}
		
		ans[n-x-1]=(bs&bs2).count();
