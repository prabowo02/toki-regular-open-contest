...
class ProblemSpec : public BaseProblemSpec {
  protected:
	...
	void Constraints() {
		...
		CONS(is1String(claps));
	}
	
  private:
    bool is1String(string s){
		for (auto it:s) if (it!='1') return false;
		return true;
	}
...
class TestSpec : public BaseTestSpec<ProblemSpec> {
  protected:
	...
	void TestGroup1() {
		CASE(n=1,q=1,moves="L",claps=genString(q));
		CASE(n=4,q=2,moves="UR",claps=genString(q));
		...
		CASE(n = maxn, q = 1, moves="L", claps=genString(q));
