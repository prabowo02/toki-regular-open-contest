...
int main() {
  ...
  for (int i = 0; i < m; ++i) {
    ...
    if (t[i] == 1) {
      // (a[x] <= z AND a[y] <= z AND a[y] > z-1) OR (a[y] <= z AND a[x] <= z AND a[x] > z-1)
      for (int x : {xz*2, yz*2, yz1*2 + 1}) for (int y : {yz*2, xz*2, xz1*2 + 1}) {
        TwoSat::addClause(x, y);
      }
    } else if (t[i] == 2) {
      // (a[x] > z-1 AND a[y] <= z AND a[y] > z-1) OR (a[y] > z-1 AND a[x] <= z AND a[x] > z-1)
      for (int x : {xz1*2 + 1, yz*2, yz1*2 + 1}) for (int y : {yz1*2 + 1, xz*2, xz1*2 + 1}) {
        TwoSat::addClause(x, y);
      }
    }
  }
