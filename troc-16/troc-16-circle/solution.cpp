...
int Solve(int N, int M, vector<int> X, vector<int> Y, vector<long long> Z) {
  ...
  const auto DivideConquer = [&](const auto &self, vector<int> &nodes, vector<int> &edges) {
    ...
    const auto Merge = [&](vector<int> first, vector<int> second, int split) {
      ...
      a.clear(), b.clear();
      for (auto i : first) {
        a.emplace_back(distX[i] - distY[i], int(distX[i] % MOD));
      }
      for (auto i : second) {
        b.emplace_back(distY[i] - distX[i], int(distX[i] % MOD));
      }
      ...
      a.clear(), b.clear();
      for (auto i : first) {
        a.emplace_back(distY[i] - distX[i], int(distY[i] % MOD));
      }
      for (auto i : second) {
        b.emplace_back(distX[i] - distY[i], int(distY[i] % MOD));
      }
      Add(res, Count([&](auto x, auto y) { return x < y; }));
